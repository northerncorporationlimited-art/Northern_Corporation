  ...
  const items = [
    {
      ...
      text: "With extensive experience in global garment manufacturing, we bring a wealth of operational knowledge to every stitch.",
      img: "/images/factory.png",
    },
    {
      title: "Uncompromising Quality",
      text: "Every thread is tested. Every seam is verified. Quality is our default setting.",
      img: "/images/leed.png",
    },
    {
      title: "Future-Focused Innovation",
      text: "We invest heavily in automated machinery and sustainable practices.",
          ...
          >
              ...
              <div className="flex flex-col gap-8 mt-8">
                {section.stats?.map((stat, idx) => (
                  <div key={idx} className="flex flex-col">
                    <span className="text-4xl md:text-5xl font-light text-brand-gold">
                      {stat.value}
                    </span>
            ...
            {section.type === "certs" && (
              <div className="mt-8 flex flex-wrap gap-3">
                <span className="px-6 py-2 border border-brand-gold text-brand-gold rounded-full text-sm tracking-widest uppercase">
                  Knitwear
                </span>
                <span className="px-6 py-2 border border-brand-gold text-brand-gold rounded-full text-sm tracking-widest uppercase">
                  Sportswear
      ...
      >
        {portfolioSections.map((section, i) => (
          <div
            key={`img-${section.id}`}
            ref={(el) => {
              if (el) imageLayersRef.current[i] = el;
            }}
